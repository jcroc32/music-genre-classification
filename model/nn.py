...
def genre_model(num_genres, input_shape):
	...
	model = MaxPooling1D(pool_size=3)(model)
	model = Dropout(0.25)(model)
	model = Conv1D(filters=64, kernel_size=7, activation='relu', dilation_rate=8)(model)
	model = MaxPooling1D(pool_size=2)(model)
	model = Dropout(0.25)(model)
	model = Conv1D(filters=32, kernel_size=7, activation='relu', dilation_rate=16)(model)
	model = MaxPooling1D(pool_size=2)(model)
	model = Dropout(0.25)(model)
	model = Conv1D(filters=32, kernel_size=5, activation='relu', dilation_rate=32)(model)
	model = MaxPooling1D(pool_size=2)(model)
	
	model = Dropout(0.25)(model)
	model = Conv1D(filters=32, kernel_size=4, activation='relu', dilation_rate=64)(model)
	model = MaxPooling1D(pool_size=2)(model)
